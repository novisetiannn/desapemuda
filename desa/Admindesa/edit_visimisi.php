...
<body>
    ...
    <h2>Edit VisiMisi</h2>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $id = $_POST['id'];
            $visi = mysqli_real_escape_string($db, $_POST['Visi']);
            $misi = mysqli_real_escape_string($db, $_POST['Misi']);

            // Update data Visi Misi ke dalam database
            $query_update = "UPDATE visi_misi SET Visi='$visi', Misi='$misi' WHERE id='$id'";
            $result_update = mysqli_query($db, $query_update);

            // Periksa apakah update berhasil
            if ($result_update) {
                echo "Data Visi Misi berhasil diupdate.";
            } else {
        ...
        if (isset($_GET['id'])) {
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                <form action="edit_visimisi.php?id=<?php echo $id; ?>" method="POST">
                    <input type="hidden" name="id" value="<?php echo $id; ?>">
                    <label for="Visi">Visi</label>
                    <textarea name="Visi" id="Visi" rows="4"><?php echo htmlspecialchars($row['Visi']); ?></textarea><br><br>
                    <label for="Misi">Misi</label>
                    <textarea name="Misi" id="Misi" rows="4"><?php echo htmlspecialchars($row['Misi']); ?></textarea><br><br>
